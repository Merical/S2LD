...
def read_megadepth_color(path, resize=None, df=None, padding=False, augment_fn=None):
    ...
    if padding:  # padding
        pad_to = max(h_new, w_new)
        image, mask = pad_bottom_right(image.cpu().numpy(), pad_to, ret_mask=True)
    else:
        mask = None
...
def read_megadepth_depth(path, resize=None, df=None, padding=False, augment_fn=None, pad_to=None):
    if str(path).startswith('s3://'):
    ...
    if resize is not None:
        ...
        if padding:  # padding
            pad = max(h_new, w_new)
            depth, _ = pad_bottom_right(depth, pad, ret_mask=False)
